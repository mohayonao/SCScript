(function() {
  ...
  describe("sc.lang.compiler.Lexer", function() {
    it("Lexer", function() {
      ...
      expect(test).to.deep.equal([ 0, 1, 0 ]);

      token = lexer.lex();
      test = lexer.getLocItems();
      expect(token.value).to.equal("a");
      expect(test).to.deep.equal([ 1, 1, 1 ]);

      token = lexer.lex();
      test = lexer.getLocItems();
      expect(token.value).to.equal("=");
      expect(test).to.deep.equal([ 3, 1, 3 ]);

      token = lexer.lex();
      test = lexer.getLocItems();
      expect(token.value).to.equal("0");
      expect(test).to.deep.equal([ 5, 1, 5 ]);
